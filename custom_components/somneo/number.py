...
async def async_setup_entry(hass, config_entry, async_add_entities):
    ...
    sunset = [SomneoSunsetTime(name, data, device_info, dev_info['serial']), SomneoSunsetBrightness(name, data, device_info, dev_info['serial'])]

    async_add_entities(alarms, True)
    async_add_entities(snooze, True)
    async_add_entities(sunset, True)
...
class SomneoSunsetTime(NumberEntity):
    _attr_should_poll = True
    _attr_available = True
    _attr_assumed_state = False
    _attr_min_value = 1
    ...
    def __init__(self, name, data, device_info, serial):
        """Initialize number entities."""
        self._data = data
        self._attr_name = name + '_sunset_time'
        self._attr_unique_id = serial + '_sunset_time'
        self._attr_value = 1
        self._attr_device_info = device_info

    def set_value(self, value: float):
        self._attr_value = value
        self._data.somneo.set_sunset_timer(int(value))

    async def async_update(self):
        """Get the latest data and updates the states."""
        await self._data.update()
        self._attr_value = self._data.somneo.sunset_timer()


class SomneoSunsetBrightness(NumberEntity):
    _attr_should_poll = True
    _attr_available = True
    _attr_assumed_state = False
    _attr_min_value = 1
    ...
    def __init__(self, name, data, device_info, serial):
        """Initialize number entities."""
        self._data = data
        self._attr_name = name + '_sunset_brightness'
        self._attr_unique_id = serial + '_sunset_brightness'
        self._attr_value = 1
        self._attr_device_info = device_info

    def set_value(self, value: float):
        self._attr_value = value
        self._data.somneo.set_sunset_brightness(int(value))

    async def async_update(self):
        """Get the latest data and updates the states."""
        await self._data.update()
        self._attr_value = self._data.somneo.sunset_brightness()
